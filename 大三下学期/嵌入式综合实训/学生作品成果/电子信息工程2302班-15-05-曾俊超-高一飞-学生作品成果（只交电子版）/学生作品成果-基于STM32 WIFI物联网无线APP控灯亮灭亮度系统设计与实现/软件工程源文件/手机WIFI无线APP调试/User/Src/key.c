...
************************************************************
*/
int KeyScan(GPIO_TypeDef* GPIOX, unsigned int NUM)
{

    if(GPIOX == KEY_GPIO_Port)
    {
        if(!HAL_GPIO_ReadPin(KEY_GPIO_Port, NUM))	//按下  为低
        {
            return KEYDOWN;
        }
        else									//弹起  为高
...
unsigned char Keyboard(void)
{
    ...
    if(KeyScan(KEY_GPIO_Port, KEY0) == KEYDOWN && !(keyBusyFlag & (~(1 << 0))))		//如果按下 且其他按键未按下
    {
    ...
    else if(KeyScan(KEY_GPIO_Port, KEY0) == KEYUP && keyBusyFlag & (1 << 0))		//如果释放 且 按键之前是按下过的
    {
        ...
        else
        {
            ...
            {
                HAL_Delay(10);												//让此任务进入阻塞态，这不影响代码正常的运行

                if(KeyScan(KEY_GPIO_Port, KEY0) == KEYDOWN)							//有第二次按下，说明为双击
                {
                    while(KeyScan(KEY_GPIO_Port, KEY0) == KEYDOWN)					//等待释放，无此句，双击后会跟一个单击动作
                        HAL_Delay(5);										//让此任务进入阻塞态，这不影响代码正常的运行
