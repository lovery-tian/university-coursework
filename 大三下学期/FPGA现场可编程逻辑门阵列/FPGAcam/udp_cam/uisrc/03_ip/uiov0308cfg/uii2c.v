...
.I(sda_o),    // Buffer input  当sda_o=0 数据从 从.I(sda_o)输入，然后输出到，到.IO(iic_sda)，总线输出0;否则数据从.IO(iic_sda)输入，然后输出到.O(sda_i)
.T(sda_o)    // 3-state enable input, high=input, low=output，当sda_o=0 数据从 从.I(sda_o)输入，然后输出到，到.IO(iic_sda)，总线输出0;否则数据从.IO(iic_sda)输入，然后输出到.O(sda_i)
);
